...
import json
from pytorch_transformers.modeling_bert import BertForClassificationFGM, BertConfig
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    level = logging.INFO)
MODEL_CLASSES = {
    'bert': (BertConfig, BertForClassificationFGM, BertTokenizer),
}
...
def read_examples(input_file, is_training,sep=','):
    ...
    examples=[]
    for val in df[['qid','question1','question2','label']].values:
...
def accuracy(out, labels):
    ...
    return f1_score(labels,outputs,labels=[0,1],average='binary')
...
def main():
    ...
    parser.add_argument("--is_first_eval", default=1, type=int,
                        help="is_do_first_eval.")
    ...
    # Prepare model
    model = BertForClassificationFGM.from_pretrained(args.model_name_or_path,args,config=config)
    ...
    if args.do_train:
        ...
        # 先做一个eval
        for file in ['dev.csv']:
            if not args.is_first_eval:
                break
            inference_labels = []
            gold_labels = []
            inference_logits = []
            eval_examples = read_examples(os.path.join(args.data_dir, file), is_training=True,sep='\t')
        ...
        for step in bar:
            ...
            if args.fp16:
                ...
                loss.backward()
                fgm.attack()
                loss_adv, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask,labels=label_ids)
    ...
    if args.do_test:
        ...
        gc.collect()
        args.do_train=False
        model = BertForClassificationFGM.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"),args,config=config)
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            if flag=='test':
                ...
                df['label_0']=logits[:,0]
                df['label_1']=logits[:,1]
                df[['qid','label_0','label_1']].to_csv(os.path.join(args.output_dir, "sub.csv"),index=False)
            if flag == 'dev':
                df = pd.read_csv(os.path.join(args.data_dir, file))
                df['label_0'] = logits[:, 0]
                df['label_1'] = logits[:, 1]
    ...
    if args.predict_eval:
        ...
        gc.collect()
        args.do_train = False
        model = BertForClassificationFGM.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"), args,
                                                              config=config)
        if args.fp16:
            model.half()
        model.to(device)
